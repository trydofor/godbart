 ...
 * `指令` 固定值，不区分大小写，当前只支持，`ENV` `REF` `RUN` `STR` `OUT`
 * `引号`包括，单引号`'`，双引号`"`，反单引号`` ` ``。
 * `空白`指英文空格`0x20`和制表符`\t`
 * `变量`和`占位`要求相同，都区分大小写。
    - ```[^ \t'"`]+``` 连续非引号空白
 ...
 * `占位`尽量使用SQL中合法数据格式，必要自找麻烦，比如没必要的引号，特殊字符等。
...
`加引号`处理，如果`变量`比`占位`多成对的引号包围，那么替换时会增加包围的引号。
因为此规则的存在，当`变量`必须带引号时，需要先`REF`在`STR`才能正确的处理。

`模式展开`，定义的`变量`和`占位`的模式若是相同， 即脱壳处理后，不被引号全包围，
